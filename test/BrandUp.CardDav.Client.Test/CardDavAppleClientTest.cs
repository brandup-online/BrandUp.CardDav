...
namespace BrandUp.CardDav.Client.Test
{
    public class CardDavAppleClientTest : TestBase
    {
        ...
        public async Task Success_Basic()
        {
            ...
            response = await client.PropfindAsync(response.ResourceEndpoints[0].Endpoint, request, Depth.One, CancellationToken.None);

            Assert.True(response.IsSuccess);

            var report = XmlQueryHelper.AddressCollection(true);

            response = await client.ReportAsync(response.ResourceEndpoints[1].Endpoint, report, Depth.One, CancellationToken.None);
        ...
        public async Task Success_CRUD()
        {
            ...
            var newUserEndpoint = response.ResourceEndpoints[1].Endpoint + "new.vcf";
            ...
            response = await client.GetAsync(newUserEndpoint, CancellationToken.None);
            Assert.True(response.IsSuccess);
